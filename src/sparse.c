...
SPARSE sparse_new(SPARSE sparse)
{
	SPARSE new;
...
void sparse_free(SPARSE sparse)
{
	free(sparse->row);
	free(sparse->column);
	free(sparse->value);
...
int sparse_solve(SPARSE sparse, double *x, double *b)
{
	...
	status = umfpack_di_symbolic(sparse->n, sparse->n, sparse->row, sparse->column, sparse->value, &symbolic, control, info);
...
int sparse_solve(SPARSE sparse, double *x, double *b)
{
	...
	phase = 11;
	PARDISO(pt, &maxfct, &mnum, &mtype, &phase, &(sparse->n), sparse->value, sparse->row, sparse->column, &idum, &nrhs, iparm, &msglvl, &ddum, &ddum, &error);
	...
	phase = 33;
	PARDISO(pt, &maxfct, &mnum, &mtype, &phase, &(sparse->n), sparse->value, sparse->row, sparse->column, &idum, &nrhs, iparm, &msglvl, b, x, &error);
	if(error != 0) return SPARSE_SOLVE_ERROR;

	// termination and release of memory
	phase = -1;
	PARDISO(pt, &maxfct, &mnum, &mtype, &phase, &(sparse->n), &ddum, sparse->row, sparse->column, &idum, &nrhs, iparm, &msglvl, &ddum, &ddum, &error);
